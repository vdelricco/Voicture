...
class FileStorageManager @Inject constructor(val context: Context) {
    ...
    fun createTempProjectAudioFiles(numFiles: Int): Observable<File> = Observable.create { subscriber ->
        repeat(numFiles) {
    ...
    fun clearTempFiles(): Completable = Completable.create { subscriber ->
    ...
    fun removeFilesFromTempList(fileList: List<File>) = tempAudioFileList.removeAll(fileList)
}